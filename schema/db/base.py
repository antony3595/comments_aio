...
class BaseSchema(BaseModel):
    ...
    def to_orm_filters(self, raise_on_wrong_attribute=True) -> Dict[str, Any]:
        ...
        assert self.model is not None, f"{self.__class__.__name__} must have a \"model\" attribute to compute filters"
        ...
        for field, value in self.model_dump(exclude_unset=True).items():
            if hasattr(self.model, field):
                ...
                raise BaseSchemaException(f"{self.__class__.__name__} field {field} is not defined")
        return filters
