...
class NewsService:
    ...
    async def subscribe_to_category(db: AsyncSession, values: NewsCategorySubscribeValues) -> List[UserCategorySubscriptionSchema]:
        async with db.begin():
            result = await get_news_repository().subscribe_to_category(db=db, values=values)
            await db.commit()
            return result
    ...
    async def get_user_subscriptions(db: AsyncSession, query: UserSubscriptionNewsQuery, pagination: PaginationSchema = None) \
        ...
        async with db.begin():
            result = await get_news_repository().get_user_subscription_news(db=db, query=query, pagination=pagination)
            await db.commit()
            return result

    async def create(self, db: AsyncSession, news_values: CreateNewsSchema) -> NewsSchema:
        async with db.begin():
            result = await get_news_repository().create(db=db, values=news_values)
            await db.commit()
            return result

    async def create_from_raw_news(self, db: AsyncSession, raw_news: RawNewsSchema) -> NewsWithCategoriesSchema | None:
        ...
        async with db.begin():
            ...
            await get_news_category_repository().create(db, news_categories_values)
            await db.commit()
    ...
    async def get_subsribers_by_categories(self, db: AsyncSession, categories: List[NewsTypeEnum]) -> List[UserSchema]:
        news_categories_repository = get_news_category_repository()
        async with db.begin():
            subscribers = await news_categories_repository.get_subscribed_users(db=db, categories=categories)
            await db.commit()
            return subscribers
