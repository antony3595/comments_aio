...
class RawNewsRepository:
    async def create(
        self, db: AsyncSession, data: CreateRawNewsSchema
    ) -> RawNewsSchema:
    ...
    async def read_all(
        ...
        return (
            [
                RawNewsSchema.model_validate(item, from_attributes=True)
                for item in result
            ]
            if result
            else []
    ...
    async def create_bulk(
        self, db, data: List[CreateRawNewsSchema]
    ) -> List[RawNewsSchema]:
        ...
        return [
            RawNewsSchema.model_validate(raw_news_item, from_attributes=True)
