...
class ServiceAccountRepository:
    ...
    async def read_expired_service_accounts(self, db: AsyncSession) -> list[ServiceAccountSchema]:
        stmt = select(ServiceAccount).where(ServiceAccount.token_valid_date <= func.now())
        db_result = await db.execute(stmt)
        accounts = db_result.scalars().all()
        return [ServiceAccountSchema.model_validate(acc, from_attributes=True) for acc in accounts] if accounts else []

    async def remove_by_id(self, db: AsyncSession, account_id: int) -> int | None:
        stmt = delete(ServiceAccount).where(ServiceAccount.id == account_id).returning(ServiceAccount.id)
        db_result = await db.execute(stmt)
